...
async function registrarVenda() {
    ...
    const vendaBase = {
        ...
        idLoja: idLoja,
        idVenda: idVenda,
        idCaixa: localStorage.getItem('caixaAtualId'),
    ...
    // 🔵 INTEGRAÇÃO COM COMANDAS (comandas.js)
    // Se essa venda começou a partir de uma comanda, fecha ela no Firestore.
    // A tela de volta e a limpeza de comandaEmPagamento ficam por conta do
    // fecharPagamento(), chamado logo abaixo — tudo centralizado em um só lugar.
    if (typeof comandaEmPagamento !== 'undefined' && comandaEmPagamento) {
        if (typeof fecharComandaAposVenda === 'function') {
            await fecharComandaAposVenda(comandaEmPagamento, idVenda);
        }
    }
...
// ─────────────────────────────────────────────
// ESTOQUE — desconto/reversão na venda e cancelamento
// (movidas de home.js pra cá, já que só o PDV mexe em estoque de venda)
// ─────────────────────────────────────────────
async function atualizarEstoque(produtos, loja) {
    ...
    try {
        ...
        snapshot.forEach(doc => {
            ...
            produtos.forEach(produto => {
                ...
                if (mesmoId || mesmoNome) {
                    // Produto sem estoque próprio (ex: unidade/dose de composição) não desconta a si mesmo
        ...
        // Desconta dos componentes da composição (kits, doses, unidades) de cada item vendido
        await Promise.all(
            produtos.map(produto =>
                ajustarEstoqueComposicao(produto.nome, produto.quantidade ?? 0, loja, -1)
            )
        );
    ...
    try {
        ...
        snapshot.forEach(doc => {
            ...
            produtos.forEach(produto => {
                ...
                if (mesmoId || mesmoNome) {
                    // Produto sem estoque próprio (ex: unidade/dose de composição) não tem o que reverter nele mesmo
                    if (dado.estoque === 'none') return;
        ...
        // Devolve aos componentes da composição (sinal +1 em vez de -1)
        await Promise.all(
            produtos.map(produto =>
                ajustarEstoqueComposicao(produto.nome, produto.quantidade ?? 0, loja, 1)
            )
...
// sinal: -1 = venda (desconta dos componentes) | +1 = reversão/cancelamento (devolve aos componentes)
// Reaproveita normalizarComposicaoParaLista() de produtos.js, que aceita tanto o
// formato antigo (objeto único) quanto o novo (lista de componentes).
async function ajustarEstoqueComposicao(nomeProdutoVendido, quantidadeVendida, idLoja, sinal) {
    const db = firebase.firestore();

    const nomeBusca = String(nomeProdutoVendido || '').toUpperCase().trim();
    if (!nomeBusca || !quantidadeVendida) return;

    try {
        const snapshotFilho = await db.collection('produtos')
            .where('idLoja', '==', idLoja)
            .where('nome', '==', nomeBusca)
            .limit(1)
            .get();

        if (snapshotFilho.empty) return; // produto vendido não está cadastrado, nada a compor

        const dadosFilho = snapshotFilho.docs[0].data();
        const componentes = normalizarComposicaoParaLista(dadosFilho.composicao);

        if (!componentes.length) return; // produto não faz parte de nenhuma composição

        await Promise.all(componentes.map(async componente => {
            const nomePai = String(componente.itemPai || '').toUpperCase().trim();
            const fracao = Number(componente.qtdComposicao) || 0;

            if (!nomePai || !fracao) return;

            const snapshotPai = await db.collection('produtos')
                .where('idLoja', '==', idLoja)
                .where('nome', '==', nomePai)
                .limit(1)
                .get();

            if (snapshotPai.empty) {
                console.warn('⚠️ Componente da composição não encontrado:', nomePai);
                return;
            }

            const paiDoc = snapshotPai.docs[0];
            const paiDados = paiDoc.data();

            if (paiDados.estoque === 'none') return; // componente também não rastreia, nada a fazer

            const estoquePaiAtual = Number(paiDados.estoque) || 0;
            const qtdAfetada = fracao * Number(quantidadeVendida);
            const novoEstoquePai = estoquePaiAtual + (sinal * qtdAfetada);

            await db.collection('produtos').doc(paiDoc.id).update({ estoque: novoEstoquePai });

            console.log(`↔️ Composição: ${paiDados.nome} ajustado em ${(sinal * qtdAfetada).toFixed(4)} (agora ${novoEstoquePai})`);
        }));

    } catch (error) {
        console.error('Erro ao ajustar estoque de composição:', error);
    }
}
...
function fecharPagamento() {
    ...
    // 🔵 INTEGRAÇÃO COM COMANDAS (comandas.js)
    // Cobre os DOIS casos: venda finalizada com sucesso OU usuário cancelou
    // o pagamento no meio do caminho. Em ambos, essa função é chamada — então
    // é o lugar certo pra decidir a tela de volta e soltar a referência da
    // comanda, sem duplicar essa checagem em mais de um lugar.
    const veioDeComanda = typeof comandaEmPagamento !== 'undefined' && comandaEmPagamento;
    if (typeof comandaEmPagamento !== 'undefined') comandaEmPagamento = null;
    ...
    if (veioDeComanda && typeof openScreen === 'function') {
        openScreen('comandas'); // volta pra Comandas em vez do PDV comum
    } else if (pdv) {
...
async function abrirResumoCaixa() {
    ...
    // ─────────────────────────────────────────────
    // CÁLCULO CORRIGIDO: separa dinheiro efetivo (sem troco)
    // dos demais meios de pagamento, venda por venda
    // ─────────────────────────────────────────────
    vendas.forEach(venda => {
        const totalVenda = Number(venda.totalVenda || 0);
        const meios = venda.meiosPagamento || [];

        // soma de tudo que NÃO é dinheiro nessa venda
        const totalOutrosMeios = meios
            .filter(mp => normalizar(mp.tipoPagamento) !== 'dinheiro')
            .reduce((acc, mp) => acc + Number(mp.valor || 0), 0);

        // quanto sobra pro dinheiro cobrir, depois dos outros meios
        const restanteParaDinheiro = Math.max(0, totalVenda - totalOutrosMeios);

        // total de dinheiro informado (pode incluir o valor usado pra dar troco)
        const totalDinheiroInformado = meios
            .filter(mp => normalizar(mp.tipoPagamento) === 'dinheiro')
            .reduce((acc, mp) => acc + Number(mp.valor || 0), 0);

        // dinheiro que REALMENTE fica no caixa (sem o troco)
        const dinheiroEfetivo = Math.min(totalDinheiroInformado, restanteParaDinheiro);

        if (totalDinheiroInformado > 0) {
            if (!porTipo['DINHEIRO']) porTipo['DINHEIRO'] = { valor: 0, qtd: 0 };
            porTipo['DINHEIRO'].valor += dinheiroEfetivo;
            porTipo['DINHEIRO'].qtd++;
        }

        // registra os outros meios, limitando ao valor real da venda
        // (protege contra inconsistência de digitação que ultrapasse o total)
        ...
        meios.forEach(mp => {
            if (normalizar(mp.tipoPagamento) === 'dinheiro') return;

            const tipo = mp.tipoPagamento;
            const valorEfetivo = Number(mp.valor || 0) * escalaOutrosMeios;

            if (!porTipo[tipo]) porTipo[tipo] = { valor: 0, qtd: 0 };
            porTipo[tipo].valor += valorEfetivo;
            porTipo[tipo].qtd++;
        });

        // total geral = soma do valor REAL das vendas, não da soma bruta de pagamentos
        totalGeral += totalVenda;
    });

    // arredonda tudo pra evitar sujeira de ponto flutuante
    Object.keys(porTipo).forEach(tipo => {
        porTipo[tipo].valor = Math.round(porTipo[tipo].valor * 100) / 100;
    });
    totalGeral = Math.round(totalGeral * 100) / 100;

    // ✅ fundo seguro contra NaN
    const fundo = parseFloat((caixa.fundo ?? '0').replace(',', '.')) || 0;
    const totalDinheiro = (porTipo['DINHEIRO'] ? porTipo['DINHEIRO'].valor : 0) + fundo;
